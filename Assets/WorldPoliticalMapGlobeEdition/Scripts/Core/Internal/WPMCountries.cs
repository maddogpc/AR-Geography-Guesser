...
namespace WPM {
    public partial class WorldMapGlobe : MonoBehaviour {
        ...
        public void RefreshCountryGeometry(Country country) {
            ...
            float dy = Mathf.Abs(maxCountry.y - minCountry.y);
            if (minCountry.y < 0 && maxCountry.y > 0 && dy > 180) {
                dy = 360 - dy;
            }
            country.regionsRect2D = new Rect(minCountry.x, minCountry.y, Math.Abs(maxCountry.x - minCountry.x), dy);
        }
